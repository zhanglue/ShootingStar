...
using ::recommendation_engine::RankingServiceImpl;
...
int main(int argc, char** argv) {
  try {
    ...
    auto ranking_logger = make_shared<Logger>(kServiceName);
    ranking_logger->SetMinLogLevel(config.GetLogLevel());
    LoggerRegistry::Register(::std::move(ranking_logger));
    ...
    RankingServiceImpl service(config);
